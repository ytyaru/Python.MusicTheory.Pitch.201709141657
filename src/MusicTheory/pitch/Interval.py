...
class Interval(metaclass=ConstMeta):
    _PatternFormat = r'(?P<prefix>[P|M|m|a|d])(?P<degree>[1-9][0-9]?)'
    _Pattern = re.compile(_PatternFormat)
    ...
    def __Split(cls, name):
        ...
        if not(match): raise ValueError(f'引数nameが有効な書式ではありません。{cls._PatternFormat}の書式にしてください。: name={name}')
#        return match.groups()['prefix'], int(match.groups()['degree'])
